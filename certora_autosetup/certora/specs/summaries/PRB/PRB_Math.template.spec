...
methods {
    ...
    function _.mul($CONTRACT_NAME$.SD59x18 x, $CONTRACT_NAME$.SD59x18 y) internal => mulDivSD59x18Summary(x,y) expect int256;
    function _.div($CONTRACT_NAME$.SD59x18 x, $CONTRACT_NAME$.SD59x18 y) internal => divSD59x18Summary(x,y) expect int256;
    
    // UD60x18 operations (unsigned decimal with 18 decimals)  
    function _.mul($CONTRACT_NAME$.UD60x18 x, $CONTRACT_NAME$.UD60x18 y) internal => mulDivUD60x18Summary(x,y) expect uint256;
    function _.div($CONTRACT_NAME$.UD60x18 x, $CONTRACT_NAME$.UD60x18 y) internal => divUD60x18Summary(x,y) expect uint256;
}
